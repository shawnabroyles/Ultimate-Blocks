...
function ub_render_star_rating_block($attributes){
    ...
    for($i = 0; $i < $attributes['starCount']; $i++){
        $stars .= $i < $attributes['selectedStars'] ?
            ub_make_full_star($starSize, $attributes['starColor'])
    ...
    return '<div class="ub-star-rating">
                ...
                <div class="ub-review-text" style="text-align:'.$attributes['reviewTextAlign'].';">'. 
...
function ub_register_star_rating_block() {
	if( function_exists( 'register_block_type' ) ) {
		register_block_type( 'ub/star-rating', array(
            'attributes' => array(
                'starCount' => array(
                    'type' => 'number',
                    'default' => 5
